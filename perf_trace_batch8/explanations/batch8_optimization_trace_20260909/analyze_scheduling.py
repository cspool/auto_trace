...
fig,axes=plt.subplots(2,1,figsize=(32,40),sharex=True)
fig.subplots_adjust(left=.075,right=.985,top=.93,bottom=.06,hspace=.16)
rect_height=.36;anchor_gap=.06
geometry={'status':'complete','layout':'two tall time plots, one per DP rank; separate vertical bands for each local B','figure_size_in':[32,40],'x_axis':'actual seconds since earliest client start','y_axis':'actual launch-local B at the dot','rectangle_display_width_s':43,'rectangle_display_height_B':rect_height,'anchor_gap_B':anchor_gap,'minimum_adjacent_B_band_gap_B':1-2*(rect_height+anchor_gap),'rectangle_width_is_measured_duration':False,'samples':[],'panels':[]}
for rank,ax in enumerate(axes):
 items=[x for x in samples if x['rank']==rank]
 ax.set_xlim(-4,239);ax.set_ylim(.5,4.5);ax.set_yticks([1,2,3,4]);ax.set_xticks(list(range(0,226,25)));ax.tick_params(labelsize=20,labelbottom=True)
 ...
 for j,x in enumerate(items):
  left=x['client_relative_s'];b=x['local_batch_sequences'];pref=x['phase']=='prefill';bottom=b+anchor_gap if pref else b-anchor_gap-rect_height
  color='#ffedbf' if pref else '#d7eafc';edge='#c88700' if pref else '#1975b6'
  patch=Rectangle((left,bottom),43,rect_height,facecolor=color,edgecolor=edge,lw=1.8,zorder=3);patch.set_gid(f'sample-rank{rank}-order{j+1}');ax.add_patch(patch)
  ax.plot([left,left],[b,bottom if pref else bottom+rect_height],color=edge,lw=2,zorder=4)
  ax.scatter([left],[b],s=105,color=edge,edgecolor='white',linewidth=.7,zorder=5)
  center=left+21.5
  texts=[ax.text(center,bottom+rect_height*.78,f'R{x["request"]:02d} {"PREFILL" if pref else "DECODE"}  |  B{b}',ha='center',va='center',fontsize=25,fontweight='bold',color='#624600' if pref else '#145985',zorder=6),
   ax.text(center,bottom+rect_height*.50,f'{left:.3f} s',ha='center',va='center',fontsize=25,color='#23374c',zorder=6),
   ax.text(center,bottom+rect_height*.22,('GQA BM'+str(x['GQA_BLOCK_M']) if x['GQA_BLOCK_M'] else 'Packed B4')+f' | {x["threads_per_block"]} threads',ha='center',va='center',fontsize=22,color='#23374c',zorder=6)]
  ...
  bounds=[left,bottom,left+43,bottom+rect_height]
 ...
 spacing_pt=ax.bbox.height/fig.dpi*72/(ax.get_ylim()[1]-ax.get_ylim()[0])
 geometry['panels'].append({'rank':rank,'xlim':list(ax.get_xlim()),'ylim':list(ax.get_ylim()),'xticks':[float(v) for v in ax.get_xticks()],'yticks':[float(v) for v in ax.get_yticks()],'ytick_spacing_pt':spacing_pt,'minimum_adjacent_B_band_gap_pt':spacing_pt*geometry['minimum_adjacent_B_band_gap_B'],'rectangles_do_not_overlap':True})
...
fig.suptitle('S  How global Batch8 becomes two independent per-device dynamic batches',x=.075,y=.985,ha='left',fontsize=28,fontweight='bold')
fig.text(.075,.965,'Actual time on X; launch-local B at each dot. Each B level has its own separated vertical band.',fontsize=23,color='#35445a')
fig.text(.075,.015,'PREFILL labels sit above their dots; DECODE labels sit below. Rectangle width = 43 display seconds (annotation size, not execution duration).\nThe dot is the measured (time, B) sample. Rectangle right edges are display boundaries. No scheduler state is interpolated between samples.',fontsize=20,linespacing=1.5,color='#35445a')
