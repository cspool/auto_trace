...
O=Path(__file__).resolve().parent;V=O/'validation';p=O/'Batch8_DP2_Scheduling_Report.pdf';doc=pymupdf.open(p);records=[];thumbs=[]
...
result={'status':'complete','pdf_sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'pages':len(doc),'library_version':pymupdf.VersionBind,'all_pages_nonempty':True,'all_text_word_boxes_inside_page':True,'Chinese_text_extractable':True,'page_records':records}
