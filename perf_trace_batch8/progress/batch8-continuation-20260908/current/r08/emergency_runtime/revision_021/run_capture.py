...
def runtime_env(passroot,segment):
    ...
    tmp=Path('/public/home/accl15ptg7/r8tmp')/('r8-'+str(os.getpid()))
...
def tracee(args):
    ...
    save(root/'control/live_collector_exec.json',{'mode':'independent_native_worker_collectors','native_collector_count':2,'service_parent_unprofiled':True,'worker_bindings_directory':str(root/'control/worker_native_collectors')})
    ...
    from r08_worker_collectors import snapshot_native_files,close_native_collectors
...
def capture(args):
    ...
    gate=read(ROOT/'raw/runtime_tools/runtime_capture_gate_014.json')
    ...
    argv=[sys.executable,'-B',str(Path(__file__)),'tracee','--pass-root',str(root),'--segment',args.segment]
    contract={'runtime_run_id':'batch8-dp2-fresh-003','runtime_goal':'R08','lineage_id':'batch8-dp2-fresh-003','segment':unit,'attempt':args.attempt,'argv':argv,'cwd':str(TARGET),'source_binding':source,'plan':source_record(ROOT/'plans/r08_capture_plan.json'),'runtime_gate':source_record(ROOT/'raw/runtime_tools/runtime_capture_gate_014.json'),'started_realtime_ns':time.time_ns(),'started_monotonic_ns':time.perf_counter_ns(),'profiling_mode':'replay_projected_attributes_only','latency_axis':'observed_R07_only','native_session_layout':'one independent native collector per DP2 GPU worker; same single service and workload'}
    ...
    with (root/'logs/hipprof.log').open('x') as log:
        ...
        save(root/'control/profiler_identity.json',{'pid':profiler.pid,'process_group':profiler.pid,'role':'unprofiled_capture_supervisor','native_collectors':2})
