...
if sys.argv[1]=='parent':
 ps=[subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct']) for d in [0]]
 for d in [0]:wait('before_stop.rank'+str(d)+'.json')
 pids=[p.pid for p in ps];snap('VISIBILITY_BEFORE_STOP.json',pids);(BASE/'ALLOW_STOP').touch()
 for d in [0]:wait('after_stop.rank'+str(d)+'.json')
 snap('VISIBILITY_AFTER_STOP.json',pids);(BASE/'ALLOW_LIBC_FLUSH').touch()
 for d in [0]:wait('after_libc_flush.rank'+str(d)+'.json')
