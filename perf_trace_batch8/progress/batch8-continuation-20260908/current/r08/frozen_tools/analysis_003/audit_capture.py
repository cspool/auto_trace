...
def main():
    ...
    for a in attrs:
        ...
        matches=conn.execute('SELECT COUNT(*) FROM '+quoted(a['replay_hip_runtime_table'])+' WHERE tid=? AND BeginNs<=? AND EndNs>=? AND (args LIKE ? OR args LIKE ? OR args LIKE ?)',(int(pmc['tid']),submitted,submitted,'hipLaunchKernel(%','hipModuleLaunchKernel(%','hipExtModuleLaunchKernel(%')).fetchone()[0]
        check(matches==1,'independently unique native launch containing CPU submission')
