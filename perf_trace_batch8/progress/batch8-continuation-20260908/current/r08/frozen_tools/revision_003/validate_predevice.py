...
def main():
    ...
    for e in ledger['handoffs'][:6]:
        ...
        for item in report['entries']:
            predecessor.verify_record({'path':item['original_path'],'size':item['size'],'sha256':item['sha256']},owner=RUN/'artifacts'/stage)
    ...
    for source in admission['verified_sources']:
        original=source.get('original_path',source.get('path'))
