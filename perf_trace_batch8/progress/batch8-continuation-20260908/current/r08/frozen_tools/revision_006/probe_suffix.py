...
def exact_native_chains(case,rows):
    ...
    for row in rows:
        ...
        query=f'SELECT rowid AS native_rowid,* FROM HIPOPS_{key} WHERE pid=? AND dev_id=? AND BeginNs=? AND EndNs=?'
        kernels=list(conn.execute(query,(int(row['pid']),int(row['gpu-id']),begin,end)))
        require(len(kernels)==1,'unique exact native hardware timestamp identity, PID, device')
        ...
        chains.append({'pmc_index':row['Index'],'pmc_queue_index':row['queue-index'],'pmc_signal':row['sig'],'pid':int(row['pid']),'native_device':int(row['gpu-id']),'pmc_queue_id':row['queue-id'],'hipops_queue_id':kernel['queue_id'],'full_kernel_literal':name,'hipops_table':'HIPOPS_'+key,'hipops_rowid':kernel['native_rowid'],'hip_runtime_index':kernel['_Index'],'hip_runtime_rowid':launch['native_rowid'],'hiptx_rowid':marker['native_rowid'],'hiptx_marker':marker['message'],'native_clock_offset_from_CONFIG':offset,'correlation_method':'Exact native hardware BeginNs and EndNs identity across formats, with CONFIG clock conversion, PID/device/full literal; preserve both native queue namespaces and unique HIPOPS row; HIPOPS _Index joins the HIP launch inside its HIPTX range.','replay_interval_overlap_used_as_join':False})
    queue_map={}
    for chain in chains:
        q=(chain['pid'],chain['native_device'],chain['pmc_queue_id'])
        require(q not in queue_map or queue_map[q]==chain['hipops_queue_id'],'consistent native queue namespace mapping')
        queue_map[q]=chain['hipops_queue_id']
