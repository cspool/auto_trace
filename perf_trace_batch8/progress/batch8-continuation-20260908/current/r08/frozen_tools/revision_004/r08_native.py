...
def output_path(value,create_parent=False):
    ...
    authorization=read(ROOT/'authorization/bulk_storage_authorization.json')
    assignment=read(ROOT/'authorization/continuation_assignment.json')
    require(sha(ROOT/'authorization/bulk_storage_authorization.json')==assignment['bulk_storage_authorization_sha256'],'bulk authorization hash')
    ...
    for lexical,destination in authorization['paths'].items():
        ...
        if p.is_relative_to(lexical):
            require(lexical.is_symlink() and os.readlink(lexical)==str(destination),'exact bulk symlink')
            require(resolved==destination/p.relative_to(lexical),'nested bulk symlink')
            require(destination.stat().st_dev==authorization['root_filesystem_device'],'bulk filesystem device')
            bound=True
    if not bound:require(resolved==p and resolved.is_relative_to(ROOT),'unlisted output symlink')
    if create_parent:
        p.parent.mkdir(parents=True,exist_ok=True)
        require(p.resolve(strict=False)==resolved,'post-creation output containment')
    return p
...
def clean_env():
    ...
    env.update(HIP_VISIBLE_DEVICES='0,1',CUDA_VISIBLE_DEVICES='0,1',PYTHONDONTWRITEBYTECODE='1',LD_LIBRARY_PATH=str(DCC)+':/opt/dtk/lib:/opt/dtk/hip/lib:/opt/dtk/rocprofiler/lib',NO_PROXY='127.0.0.1,localhost',no_proxy='127.0.0.1,localhost')
    return env
