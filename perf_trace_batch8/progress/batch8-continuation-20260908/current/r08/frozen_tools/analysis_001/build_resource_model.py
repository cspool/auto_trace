...
def main():
    ...
    fields=['schema_version','runtime_run_id','lineage_id','trace_profile_sha256','metric_id','record_kind','r07_process_range_id','r07_bound_target_id','r07_kernel_instance_id','request_id','dp_rank','native_device','logical_family_ids','physical_attribute_id','counter_mode','metric_name','value','unit','formula','assumptions','aggregation_rule','rule_version','evidence_class','availability_state','availability_reason','source_path','source_sha256','source_row_id','capability_path','capability_sha256']
    ...
    with path.open('x',newline='') as f:
        ...
        for a in sorted(attrs,key=lambda a:(a['r07_kernel_instance_id'],a['counter_mode'])):
            base={k:a[k] for k in ['r07_process_range_id','r07_bound_target_id','r07_kernel_instance_id','request_id','dp_rank','native_device','physical_attribute_id','counter_mode']};base.update(record_kind='native_dispatch_metric',logical_family_ids=json.dumps(a['logical_family_ids'],separators=(',',':')),source_path=a['raw_csv_path'],source_sha256=a['raw_csv_sha256'],source_row_id=a['physical_attribute_id'])
