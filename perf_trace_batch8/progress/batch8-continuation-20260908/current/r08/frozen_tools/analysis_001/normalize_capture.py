...
def main():
    ...
    for source_id,selection in pending.items():
        ...
        for field in ['request_id','phase','phase_occurrence','dp_rank','physical_device_id','layer_idx','process_id','fragment_id','q_len','kv_len']:
    ...
    for number,row in enumerate(raw,2):
        ...
        key=(int(row['pid']),int(row['gpu-id']),int(row['BeginNs']),int(row['EndNs']))
