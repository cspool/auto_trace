...
def main():
    ...
    for pid in pid_ranks:
        ...
        sql='SELECT rowid AS native_rowid,* FROM '+q('HIPOPS_'+ck)+' WHERE Name IN ('+','.join('?' for _ in ids)+')'
        for op in conn.execute(sql,ids):
            ...
            h=dict(hs[0]);require(str(h['args']).split('(',1)[0] in LAUNCH_APIS,'native direct launch API')
    ...
    missing=set(eligible)-used
    diagnostic=[]
    for k in sorted(missing):
        n=eligible[k];pid=n['kernel']['pid'];h=n['launch'];offset=n['native_clock_offset'];candidates=[(abs(int(row['DispatchNs'])+offset-h['BeginNs']),row) for row in raw if int(row['pid'])==pid and int(row['tid'])==h['tid']]
        closest=sorted(candidates,key=lambda x:x[0])[:3]
        diagnostic.append({'eligible_key':k,'rank':n['process']['dp_rank'],'request_id':n['process']['request_id'],'phase':n['process']['phase'],'layer_idx':n['process']['layer_idx'],'launch':h,'kernel':n['kernel'],'r07':n['r07'],'nearest_rows_diagnostic_only_no_attribution':[{'difference_from_launch_begin_ns':distance,'row':r} for distance,r in closest]})
    save(output/'MISSING_PMC_DIAGNOSTIC.json',{'status':'diagnostic_only_not_promotable','expected':len(eligible),'accepted':len(accepted),'accepted_rank_counts':dict(collections.Counter(x['dp_rank'] for x in accepted)),'raw_rank_counts':dict(collections.Counter(x['gpu-id'] for x in raw)),'missing':diagnostic})
