...
def _event_payload(operation: str, rank: int | None) -> dict[str, Any]:
    return {
        ...
        "control_backend": "hipProfilerStop_Start_with_offline_pid_monotonic_guard",
...
def _ensure_process_initialized() -> None:
    ...
    with _lock:
        ...
        _stop_event = None
        _watcher_pid = None
        transition = _hip_call("hipProfilerStop")
...
def install(r01_module: Any) -> None:
    ...
    print(
        ...
        "measured_requests=8 collection_state=initially_stopped "
        "canonical_window=dynamic_plus_offline_guard",
        flush=True,
    )
