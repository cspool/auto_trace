...
def capture(args):
    ...
    gate=read(ROOT/'validation/runtime_capture_gate.json')
    require(gate['status']=='complete','runtime capture gate')
    require(read(ROOT/'preflight/source_abi_probe.json')['status']=='complete','separate source/ABI/device checkpoint')
    ...
    argv=collector_argv(unit['mode'],unit['collector_kernel_name_token'],root,[sys.executable,'-B',str(Path(__file__)),'tracee','--pass-root',str(root),'--segment',args.segment],disabled=True)
    contract={'runtime_run_id':'batch8-dp2-fresh-003','runtime_goal':'R08','lineage_id':'batch8-dp2-fresh-003','segment':unit,'attempt':args.attempt,'argv':argv,'cwd':str(TARGET),'source_binding':source,'plan':source_record(ROOT/'plans/r08_capture_plan.json'),'runtime_gate':source_record(ROOT/'validation/runtime_capture_gate.json'),'started_realtime_ns':time.time_ns(),'started_monotonic_ns':time.perf_counter_ns(),'profiling_mode':'replay_projected_attributes_only','latency_axis':'observed_R07_only'}
