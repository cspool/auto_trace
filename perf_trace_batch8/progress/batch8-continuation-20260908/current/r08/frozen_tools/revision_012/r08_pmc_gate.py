...
def _event_payload(operation: str, rank: int | None) -> dict[str, Any]:
    return {
        ...
        "control_backend": "always_on_native_collection_with_exact_marker_PID_dispatch_guard",
...
def _stop_if_started(reason: str) -> None:
    ...
    with _lock:
        ...
        _write_summary()
        profiler_transition = _logical_window_transition("logical_window_stop")
        _stopped = True
        ...
        if measured:
            with _lock:
                if not _started:
                    ...
                    profiler_transition = _logical_window_transition("logical_window_start")
                    _started = True
                    _rank = rank
                    event_root = _required_path("QWEN_DCU_R08_PMC_GATE_EVENT_ROOT")
