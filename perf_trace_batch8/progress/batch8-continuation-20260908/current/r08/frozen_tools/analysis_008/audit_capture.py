...
def main():
    ...
    native_union=load_native_union(capture,verify_all=True)
    ...
    seen=set();pmc_seen=set();native_seen=set();counts=collections.Counter();queues={};selected_ranges=collections.defaultdict(set)
    dbsha=sha(capture/'capture.db');csvsha=sha(capture/'capture.csv')
    for a in attrs:
        for key,value in source_fields(native_union,int(a['pmc_pid']),int(a['pmc_csv_row'])).items():check(a.get(key)==value,'original native session provenance '+key)
        ...
        selected_ranges[(int(a['pmc_pid']),tx['tid'])].add((tx['begin_Index'],tx['end_Index']))
