...
def main():
    ...
    save(output/'NATIVE_PMC_COVERAGE_DIAGNOSTIC.json',{'expected':len(eligible),'used':len(used),'raw':len(raw),'missing':[{'signature':list(k),'launch':v['launch'],'kernel':v['kernel'],'process':v['process'],'r07':v['r07']} for k,v in eligible.items() if k not in used],'accepted_diagnostic_only':accepted}); return
