...
def main():
 ...
 for kind,items in [('request',request),('process',process),('strict_owned_kernel',kernel),('gpu_queue',kernel)]:
  ...
  for r in sorted(items,key=lambda r:(int(r['begin_ns']),int(r['end_ns']),r.get('kernel_instance_id',r.get('process_range_id',r['request_id'])))):
   ...
   lanes[kind,sid]=free
 count_expected=len(request)+len(process)+2*len(kernel);check(ACCEPT.is_symlink() and os.readlink(ACCEPT)==assignment['bulk_storage_paths'][str(ACCEPT)] and not any(ACCEPT.iterdir()),'exact empty assigned acceptance bulk entrypoint');check(ACCEPT.stat().st_dev==assignment['NFS_filesystem_device'],'acceptance physically on NFS');counts=collections.Counter();row_identity={};trace=ACCEPT/'E2E_PROCESS_TIMELINE.full.perfetto.json';first=True
