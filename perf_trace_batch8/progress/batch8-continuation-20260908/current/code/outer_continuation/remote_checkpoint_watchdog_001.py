...
CONTROL=Path('/public/home/accl15ptg7/run_R08_R10');PROJECT=Path('/public/home/accl15ptg7/auto_trace');R08=PROJECT/'perf_trace_batch8/runtime/workflow01-10-fresh-e2e/batch8-dp2-fresh-003/artifacts/R08/continuation_001';WORKTREE=CONTROL/'remote_progress_checkout';REL=Path('perf_trace_batch8/progress/batch8-continuation-20260908');DEST=WORKTREE/REL
...
def redact(b):
 ...
 return text.encode()
def snapshot():
 begun=now();tmp=DEST/'next_snapshot';tmp.mkdir(parents=True,exist_ok=True);records=[]
 ...
 (CONTROL/'REMOTE_PROGRESS_LAST_PUSH.json').write_text(json.dumps(result,indent=2)+'\n');print('REMOTE_PROGRESS_PUSHED',result['utc'],commit,len(accepted),flush=True)
