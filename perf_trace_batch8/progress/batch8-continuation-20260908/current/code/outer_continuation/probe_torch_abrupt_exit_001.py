...
script=ROOT/'tools/gate_probe_001/torch_abrupt_exit_probe.py';save(ROOT/'validation/torch_abrupt_exit_probe_cpu_gate.json',{'status':'complete','script':source_record(script),'model_imports':0,'model_initializations':0,'device_work':'two workers x four torch add kernels; no Qwen initialization','source':source_record(Path(__file__))})
base=output_path(ROOT/'preflight/torch_abrupt_exit_probe_001');base.mkdir();env=clean_env();env.pop('PYTHONPATH',None)
results=[]
for label,off,trace in [('abrupt_trace',True,True),('abrupt_pmc_only',False,False)]:
