...
base=R/'raw/runtime_tools/native_PMC_ROCR_masked_probe_014';base.mkdir();(base/'hipprof_tmp').mkdir();original=(R/'raw/runtime_tools/native_fresh_exec_pmc_write_probe_001/gqa6_native_probe.py').read_text();worker=original[original.index('else:\n')+len('else:\n'):]
...
'''
script=base/'gqa6_visibility_probe.py';header=header.replace("ps=[subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct']) for d in [0,1]]", "ps=[subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct'],env={**os.environ,'ROCR_VISIBLE_DEVICES':str(d),'HIP_VISIBLE_DEVICES':'0','CUDA_VISIBLE_DEVICES':'0'}) for d in [0,1]]");worker=worker.replace('torch.cuda.set_device(d)','torch.cuda.set_device(0)');worker=worker.replace(' lib.hipProfilerStop();q=', " write('device_observation.rank'+str(d)+'.json',{'rank':d,'pid':os.getpid(),'visible_count':torch.cuda.device_count(),'current_device':torch.cuda.current_device(),'device_properties':str(torch.cuda.get_device_properties(0))})\n lib.hipProfilerStop();q=");script.write_text(header+worker);compile(script.read_text(),str(script),'exec');save(base/'CPU_GATE.json',{'status':'complete','script':source_record(script),'prior_failed_capture':source_record(A/'RAW_CAPTURE_FAILURE.json'),'prior_groups_verified_closed':groups,'model_initializations':0,'four_measured_gqa6_dispatches_per_device_only':True,'native_counter_mode':'pmc_write','not_R08_model_measurement_evidence':True})
