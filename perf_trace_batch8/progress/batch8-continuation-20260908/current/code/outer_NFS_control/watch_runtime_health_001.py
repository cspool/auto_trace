...
def observe():
 ...
 fs=os.statvfs(C);offloads=list((R/'raw/runtime_tools/remote_release_offloads_001').glob('*.complete.json'));result={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'accepted_R08':len(accepted),'planned_R08':12,'current_capture':str(root.relative_to(R/'raw/captures')),'current_phase':phase,'native_workers':native,'pre_measured_health':health['status'] if health else 'pending','NFS_available_GiB':round(fs.f_bavail*fs.f_frsize/(1<<30),2),'temporarily_evicted_bytes':sum(read(p)['evicted_bytes'] for p in offloads),'remaining_machine_hours':round((DEADLINE-time.time())/3600,2),'completed_stages':[stage for stage in ['R08','R09','R10'] if (RUN/'handoffs'/(stage+'.continuation.json')).exists()]}
