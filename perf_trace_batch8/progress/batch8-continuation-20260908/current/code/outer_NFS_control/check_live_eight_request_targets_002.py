...
def main():
 seg,attempt=sys.argv[1:];root=R/'raw/captures'/seg/attempt;reference=REFERENCE.read_bytes();reference_rows=json.loads(reference)['records'];expected=collections.defaultdict(set)
 for x in reference_rows:
  if 'source_r06_target_id' in x:expected[x['request_id'],x['phase']].add(x['canonical_logical_selection_id'])
 observed=collections.defaultdict(list);sources=[];pids={};representatives=[]
 for p in sorted((root/'workload/overlay_events').glob('rank*/*.jsonl')):
  ...
  for x in records:
   ...
   if x['layer_idx']==0 and x['process_id']=='input_rmsnorm':representatives.append(x)
 ...
 for phase in ['prefill','decode']:
  for rank in [0,1]:
   key=phase+'.rank'+str(rank)
   old=[x for x in reference_rows if x['marker_kind']=='process' and x['phase']==phase and x['layer_idx']==0 and x['process_id']=='input_rmsnorm' and x['dp_rank']==rank]
   current=[x for x in representatives if x['phase']==phase and x['dp_rank']==rank]
   expected_order[key]=[x['request_id'] for x in sorted(old,key=lambda x:x['runtime_execution_id'])]
   actual_order[key]=[x['request_id'] for x in sorted(current,key=lambda x:x['runtime_execution_id'])]
