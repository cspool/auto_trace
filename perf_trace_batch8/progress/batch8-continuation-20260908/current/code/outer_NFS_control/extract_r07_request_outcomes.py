...
selected={f['path']:f for f in manifest['files'] if '/R07/resume-042/capture/workload/' in f['path'] and Path(f['path']).name in ['driver.json','request_results.json','warmup_results.json'] and f['coverage']['type']=='supplement_file'}
print('STREAM_VERIFIED',len(selected),'selected tools/contracts',flush=True)
dest=C/'r07_additional_inputs'
...
with (C/'r07_additional_inputs_restore_manifest.json').open('x') as f:json.dump({'status':'complete','stream_sha256':stream.hexdigest(),'entries':found},f,indent=2);f.write('\n')
