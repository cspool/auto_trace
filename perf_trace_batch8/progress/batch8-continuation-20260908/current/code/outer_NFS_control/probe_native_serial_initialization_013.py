...
base=R/'raw/runtime_tools/native_PMC_serial_initialization_probe_013';base.mkdir();(base/'hipprof_tmp').mkdir();original=(R/'raw/runtime_tools/native_fresh_exec_pmc_write_probe_001/gqa6_native_probe.py').read_text();worker=original[original.index('else:\n')+len('else:\n'):]
...
header=header.replace(" ps=[subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct']) for d in [0,1]]", " ps=[]\n for d in [0,1]:\n  ps.append(subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct']));wait('warm_ready.rank'+str(d)+'.json')\n (BASE/'ALLOW_START').touch()\n for d in [0,1]:wait('initial_started.rank'+str(d)+'.json')\n (BASE/'ALLOW_TARGET').touch()")
