...
def main():
 ...
 for ordinal,unit in enumerate(read(ROOT/'plans/r08_capture_plan.json')['physical_captures']):
  ...
  attempt='attempt_002' if ordinal==3 else 'attempt_001';raw=ROOT/'raw/captures'/seg/attempt
  assert time.time()<DEADLINE-1800,'bounded full capture reserve'
  fs=os.statvfs(ROOT);free=fs.f_bavail*fs.f_frsize
  assert free>=10*(1<<30),'NFS capacity insufficient for next capture; accepted released files may be evicted'
