...
def main():
 ...
 tmp=Path('/public/home/accl15ptg7/cputmp')/(a.phase+'-'+str(os.getpid()));tmp.mkdir(parents=True);env.update(TMPDIR=str(tmp),XDG_CACHE_HOME=str(tmp/'cache'),XDG_CONFIG_HOME=str(tmp/'config'));assert tmp.stat().st_dev==root.stat().st_dev,'CPU temporary output on NFS'
