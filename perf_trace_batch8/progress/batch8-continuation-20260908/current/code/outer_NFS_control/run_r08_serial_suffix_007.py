...
def main():
 ...
 SCHEDULE.mkdir();save(SCHEDULE/'START.json',{'pid':os.getpid(),'source':rec(Path(__file__)),'deadline_utc':'2026-09-08T20:18:09Z','deadline_authorization':rec(Path('/public/home/accl15ptg7/run_R08_R10/MACHINE_TIME_EXTENSION_001.json')),'previous_scheduler':rec(ROOT/'validation/serial_scheduler_006/START.json'),'resume_reason':'reuse four audited captures; fourth raw reused unchanged after offline graph-background classification repair','CPU_only_repair_authorization':rec(ROOT/'validation/cpu_attribution_repair_001/AUTHORIZATION.json'),'NFS_storage_authorization':rec(ROOT/'authorization/NFS_output_storage_authorization_001.json'),'user_storage_policy':rec(Path('/public/home/accl15ptg7/run_R08_R10/USER_STORAGE_POLICY_20260908.json')),'scope':'R08 captures and CPU attribution; no R09/R10 business execution'})
 accepted=[]
 for ordinal,unit in enumerate(read(ROOT/'plans/r08_capture_plan.json')['physical_captures']):
  seg=unit['segment_id']
  if ordinal<4:
   previous=read(ROOT/('validation/serial_scheduler_006' if ordinal<3 else 'validation/cpu_attribution_repair_001')/(seg+'.accepted_checkpoint.json'));assert previous['status']=='accepted';item=previous['item']
