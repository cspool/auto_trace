...
def main():
 for gate in [ROOT/'raw/runtime_tools/runtime_capture_gate_011.json', ROOT/'validation/NFS_mmap_analysis_CPU_gate_001.json']:
  g=read(gate);assert g['status']=='complete'
  for x in g['frozen_tools']:assert sha(x['path'])==x['sha256'],'frozen tool bytes'
 SCHEDULE.mkdir();save(SCHEDULE/'START.json',{'pid':os.getpid(),'source':rec(Path(__file__)),'deadline_utc':'2026-09-08T20:18:09Z','deadline_authorization':rec(Path('/public/home/accl15ptg7/run_R08_R10/MACHINE_TIME_EXTENSION_001.json')),'previous_scheduler':rec(ROOT/'validation/serial_scheduler_005/START.json'),'resume_reason':'reuse three accepted published captures; fourth native raw valid but non-equivalent kernel ownership; replay R07 observed per-rank scheduler admission order','native_retry_authorization':rec(ROOT/'raw/runtime_tools/capture04_retry_authorization_003.json'),'NFS_storage_authorization':rec(ROOT/'authorization/NFS_output_storage_authorization_001.json'),'user_storage_policy':rec(Path('/public/home/accl15ptg7/run_R08_R10/USER_STORAGE_POLICY_20260908.json')),'scope':'R08 captures and CPU attribution; no R09/R10 business execution'})
 accepted=[]
 for ordinal,unit in enumerate(read(ROOT/'plans/r08_capture_plan.json')['physical_captures']):
  seg=unit['segment_id']
  if ordinal<3:
   previous=read(ROOT/'validation/serial_scheduler_004'/(seg+'.accepted_checkpoint.json'));assert previous['status']=='accepted';item=previous['item']
  ...
  attempt='attempt_003' if ordinal==3 else 'attempt_001';raw=ROOT/'raw/captures'/seg/attempt
  ...
  output=ROOT/'normalized/captures'/seg/attempt/'revision_002';phase([sys.executable,'-B',str(ROOT/'tools/analysis_006/normalize_capture.py'),'--segment',seg,'--attempt',attempt,'--revision','revision_002'],SCHEDULE/(seg+'.normalize.log'));phase([sys.executable,'-B',str(ROOT/'tools/analysis_006/audit_capture.py'),'--normalization',str(output)],SCHEDULE/(seg+'.audit.log'))
