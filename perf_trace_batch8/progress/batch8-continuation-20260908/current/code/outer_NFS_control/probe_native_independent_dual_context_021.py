...
base=R/'raw/runtime_tools/native_PMC_independent_dual_context_probe_021';base.mkdir();source=R/'raw/runtime_tools/native_PMC_visibility_probe_001/gqa6_visibility_probe.py';processes=[];results=[]
for d in [0,1]:
 unit=base/('rank'+str(d));unit.mkdir();(unit/'hipprof_tmp').mkdir();script=unit/'gqa6_native_probe.py';script.write_text(source.read_text().replace('BASE=Path(__file__).parent','BASE=Path(__file__).parent.parent').replace(' graph=None',' torch.cuda.set_device(1-d);other_context_tensor=torch.randn((1,),device="cuda");torch.cuda.synchronize();torch.cuda.set_device(d)\n graph=None'));env=clean_env();env.pop('PYTHONPATH',None);env.update(TRITON_CACHE_DIR=str(R/'raw/runtime_cache/triton'),PYTHONPYCACHEPREFIX=str(R/'raw/runtime_cache/pycache'),PYTHONDONTWRITEBYTECODE='1');argv=collector_argv('pmc_write','_gqa6',unit,[sys.executable,'-B',str(script),'worker',str(d),'direct'],trace=True,disabled=True);log=(unit/'collector.log').open('x');p=subprocess.Popen(argv,cwd=unit,env=env,stdout=log,stderr=subprocess.STDOUT,start_new_session=True);processes.append((d,unit,p,log,argv))
