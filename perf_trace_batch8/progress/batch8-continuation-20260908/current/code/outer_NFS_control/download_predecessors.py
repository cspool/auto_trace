...
def download(item):
    ...
    # Resolve GitHub through the working origin route, then fetch its signed CDN
    # asset through the independently measured high-throughput site route.
    session.trust_env = False
    session.proxies = {'http': os.environ['ftp_proxy'], 'https': os.environ['ftp_proxy']}
    download_url = None
    for attempt in range(80):
        try:
            if download_url is None:
                with origin_session.get(a['url'], headers={'Range': 'bytes=0-0'}, stream=True, timeout=(30, 60)) as redirect:
                    redirect.raise_for_status()
                    download_url = redirect.url
