...
base=R/'raw/runtime_tools/native_PMC_synchronized_masked_probe_010';base.mkdir();(base/'hipprof_tmp').mkdir();original=(R/'raw/runtime_tools/native_fresh_exec_pmc_write_probe_001/gqa6_native_probe.py').read_text();worker=original[original.index('else:\n')+len('else:\n'):]
...
'''
worker=worker.replace(' lib.hipProfilerStop();q=', " assert lib.hipProfilerStop()==0\n write('initial_stopped.rank'+str(d)+'.json',{'rank':d,'pid':os.getpid()});wait('ALLOW_WARMUP')\n q=")
worker=worker.replace(' assert lib.hipProfilerStart()==0', " write('warm_ready.rank'+str(d)+'.json',{'rank':d,'pid':os.getpid()});wait('ALLOW_START')\n assert lib.hipProfilerStart()==0\n write('initial_started.rank'+str(d)+'.json',{'rank':d,'pid':os.getpid()});wait('ALLOW_TARGET')")
header=header.replace(" for d in [0,1]:wait('before_stop.rank'+str(d)+'.json')", " for d in [0,1]:wait('initial_stopped.rank'+str(d)+'.json')\n (BASE/'ALLOW_WARMUP').touch()\n for d in [0,1]:wait('warm_ready.rank'+str(d)+'.json')\n (BASE/'ALLOW_START').touch()\n for d in [0,1]:wait('initial_started.rank'+str(d)+'.json')\n (BASE/'ALLOW_TARGET').touch()\n for d in [0,1]:wait('before_stop.rank'+str(d)+'.json')")
header=header.replace("ps=[subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct']) for d in [0,1]]", "ps=[subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct'],env={**os.environ,'HIP_VISIBLE_DEVICES':str(d),'CUDA_VISIBLE_DEVICES':str(d)}) for d in [0,1]]")
worker=worker.replace('torch.cuda.set_device(d)','torch.cuda.set_device(0)').replace('torch.cuda.set_device(1-d)','torch.cuda.set_device(0)')
