...
def main():
 ...
 for ordinal,unit in enumerate(read(ROOT/'plans/r08_capture_plan.json')['physical_captures']):
  ...
  attempt='attempt_003' if ordinal==2 else 'attempt_001';raw=ROOT/'raw/captures'/seg/attempt
  if ordinal==2:
   begin=time.time()
   while not (raw/'execution_manifest.json').exists():
    assert time.time()-begin<3600 and time.time()<DEADLINE-120,'active retry raw checkpoint timeout'
    if (raw/'FAILURE.json').exists() or (raw/'RAW_CAPTURE_FAILURE.json').exists():raise RuntimeError('active retry failed, preserve attempt')
    time.sleep(5)
  else:
   assert time.time()<DEADLINE-1800,'bounded full capture reserve'
   fs=os.statvfs(ROOT);free=fs.f_bavail*fs.f_frsize
   if free<8*(1<<30):
    save(SCHEDULE/(seg+'.NFS_CAPACITY_PAUSE.json'),{'status':'paused_for_NFS_capacity','free_bytes':free,'required_capture_reserve_bytes':8*(1<<30),'next_segment':seg,'user_storage_policy':'all R08-R10 artifacts/logs physically NFS; weights physical root','accepted_prefix_count':len(accepted)});raise RuntimeError('NFS capacity insufficient for next complete capture')
  ...
  output=ROOT/'normalized/captures'/seg/attempt/'revision_002';phase([sys.executable,'-B',str(ROOT/'tools/analysis_005/normalize_capture.py'),'--segment',seg,'--attempt',attempt,'--revision','revision_002'],SCHEDULE/(seg+'.normalize.log'));phase([sys.executable,'-B',str(ROOT/'tools/analysis_005/audit_capture.py'),'--normalization',str(output)],SCHEDULE/(seg+'.audit.log'))
