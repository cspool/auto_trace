...
def main():
 ...
 run([PY,'-B',str(CONTROL/'verify_completed_R08_restoration_001.py')])
 # R08 owns resource construction and closure; R09 is never assigned early.
 run([PY,'-B',str(CONTROL/'prepare_stage_assignments_004.py'),'assign','R08']);phase(r08,'resource_model',r08/'tools/analysis_006/build_resource_model.py');phase(r08,'resource_audit',r08/'tools/analysis_006/audit_resource_model.py');phase(r08,'seal',r08/'tools/closure_001/seal_r08.py');phase(r08,'completion_audit',r08/'tools/closure_001/audit_r08.py');run([PY,'-B',str(CONTROL/'prepare_stage_assignments_004.py'),'handoff','R08'])
 for stage,phases in [('R09',[('analysis','build_analysis.py'),('table_audit','audit_analysis.py'),('seal','seal_r09.py'),('completion_audit','audit_completion.py')]),('R10',[('render','build_report.py'),('browser','browser_acceptance.py'),('seal','seal_acceptance.py'),('completion_audit','audit_report.py')])]:
  run([PY,'-B',str(CONTROL/'prepare_stage_assignments_004.py'),'assign',stage]);root=RUN/'artifacts'/stage/'continuation_001'
  for name,tool in phases:phase(root,name,root/'tools/revision_001'/tool)
  run([PY,'-B',str(CONTROL/'prepare_stage_assignments_004.py'),'handoff',stage])
