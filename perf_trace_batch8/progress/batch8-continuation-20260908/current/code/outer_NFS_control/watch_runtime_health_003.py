...
def observe():
 ...
 if len(tokens)==8 and min(tokens.values())>=2 and phase=='measured_batch':
  ...
  if not complete:
   p=subprocess.run([sys.executable,'-B',str(C/'check_live_eight_request_targets_001.py'),x['segment']['segment_id'],root.name],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,timeout=30)
