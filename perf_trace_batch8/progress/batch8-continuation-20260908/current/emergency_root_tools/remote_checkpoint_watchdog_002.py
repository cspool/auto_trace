...
CONTROL=Path('/public/home/accl15ptg7/run_R08_R10');PROJECT=Path('/public/home/accl15ptg7/auto_trace');R08=PROJECT/'perf_trace_batch8/runtime/workflow01-10-fresh-e2e/batch8-dp2-fresh-003/artifacts/R08/continuation_001';WORKTREE=Path('/root/remote_progress_checkout_002');REL=Path('perf_trace_batch8/progress/batch8-continuation-20260908');DEST=WORKTREE/REL
...
def snapshot():
 ...
 for p in sorted(Path('/root/r08_emergency_tools').glob('*')):
  if p.is_file() and p.suffix in ['.py','.json','.log','.md']:copy(p,Path('emergency_root_tools')/p.name,'log_snapshot' if p.suffix=='.log' else 'emergency_recovery_code_or_state')
 ...
 for p in sorted((R08/'raw/runtime_tools/serial_scheduler_002').glob('*.accepted_checkpoint.json')):
 ...
 tmp.rename(current)
 (DEST/'README.md').write_text('# Batch8 DP2 容器丢失恢复检查点\n\n每10分钟提交一次当前状态、代码和完整日志快照；Git历史保留旧检查点。当前代码模板明确标记为未执行，不能作为R09/R10完成证据。每个通过独立审计的R08采集另发原始数据Release。\n\n恢复先读 `current/SNAPSHOT_MANIFEST.json` 和 `current/state/session_state.json`，再核对 `current/r08/accepted` 与 `current/remote_publications`。只有原始数据、规范化清单和独立审计均匹配的采集才能复用；未完成采集只保留日志，不算通过。\n\n截止时间：2026-09-08 20:18:09 UTC。\n')
 ...
 (Path('/root/r08_emergency_tools')/'REMOTE_PROGRESS_LAST_PUSH.json').write_text(json.dumps(result,indent=2)+'\n');print('REMOTE_PROGRESS_PUSHED',result['utc'],commit,len(accepted),flush=True)
