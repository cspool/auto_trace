...
def main():
 ...
 for ordinal,unit in enumerate(read(ROOT/'plans/r08_capture_plan.json')['physical_captures']):
  ...
  if ordinal==2:
   ...
   assert time.time()<DEADLINE-1800,'bounded full capture reserve';phase([sys.executable,'-B',str(RUNTIME/'run_capture.py'),'capture','--segment',seg,'--attempt',attempt],SCHEDULE/(seg+'.capture.log'))
  ...
  output=ROOT/'normalized/captures'/seg/attempt/'revision_002';phase([sys.executable,'-B',str(ROOT/'tools/analysis_004/normalize_capture.py'),'--segment',seg,'--attempt',attempt,'--revision','revision_002'],SCHEDULE/(seg+'.normalize.log'));phase([sys.executable,'-B',str(ROOT/'tools/analysis_004/audit_capture.py'),'--normalization',str(output)],SCHEDULE/(seg+'.audit.log'))
