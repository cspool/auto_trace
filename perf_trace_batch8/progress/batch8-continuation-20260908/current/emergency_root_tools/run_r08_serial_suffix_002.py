...
def main():
 ...
 for ordinal,unit in enumerate(read(ROOT/'plans/r08_capture_plan.json')['physical_captures']):
  ...
  attempt='attempt_002' if ordinal==2 else 'attempt_001';raw=ROOT/'raw/captures'/seg/attempt
  if ordinal==2:
   begin=time.time()
   while not (raw/'execution_manifest.json').exists():
    ...
    if (raw/'FAILURE.json').exists():raise RuntimeError('active retry failed, preserve attempt')
    time.sleep(5)
  else:
