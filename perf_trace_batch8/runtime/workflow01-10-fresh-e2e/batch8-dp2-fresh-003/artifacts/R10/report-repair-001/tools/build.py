...
def main():
 ...
 original=ROOT/'original';accept=ROOT/'acceptance';assert not original.exists() and not accept.exists(),'Use a new empty output root or retain failed attempt before rebuilding'
 shutil.copytree(SOURCE,original);accept.mkdir()
 css=(TOOLS/'ranked.css').read_text();code=(TOOLS/'ranked.js').read_text();viewer=(SOURCE/'tools/revision_001/viewer.js').read_text();base_viewer=viewer[:viewer.index('(async()=>{try{const tables=await loadTables();')]
 ...
 for name in ['HIGH_LATENCY_PROCESS_HARDWARE_TIMELINE.html','CONCURRENCY_UTILIZATION.html']:
  ...
  intro='<section class="rank-intro"><h2>排名时间线 · 展示修订 1</h2><p>默认前 20 项/组，可定位任意排名或显示全部。高延迟组可切换所有组内实例；可放大、平移、精确纳秒跳转，点击查看源记录。排名不删减数据，原始完整 R10 另行保留。</p><p>所有横轴来自 R07 观测。蓝色为观测过程；双卡并发是 R07 派生计数；绿色点为原始利用率；紫色斜纹为未知；黄色虚线框为 R08 重放/静态硬件旁证。后者不代表 R07 实测利用率、根因或加速比。</p></section><div id="rankedRoot"></div>'
