#!/usr/bin/env python3
"""Explicit per-worker native start before the original warmup and measured batch.
...
def _event_payload(operation: str, rank: int | None) -> dict[str, Any]:
    return {
        ...
        "control_backend": "native_initially_off_warmup_stop_start_measured_start_terminal_stop",
...
def _ensure_process_initialized() -> None:
    """Reset logical marker state after a multiprocessing fork."""
...
def _start_before_original_warmup(runner: Any, scheduler_output: Any) -> None:
    ...
    with _lock:
        ...
        # Establish a real off-to-on native transition after model initialization.
        ...
        payload.update(profiler_reset_stop_transition=reset_transition,
                       native_off_to_on_reset_before_original_warmup=True)
...
def install(r01_module: Any) -> None:
    global _r01, _original_native_push, _installed, _MEASURED_REQUEST_IDS, _WARMUP_REQUEST_IDS, _process_pid
    if _installed:
        return
    ...
    _WARMUP_REQUEST_IDS = [f"r08-pmc-warmup-{i+1:03d}-{manifest['records'][i]['request_id']}" for i in range(2)]
    _r01 = r01_module
