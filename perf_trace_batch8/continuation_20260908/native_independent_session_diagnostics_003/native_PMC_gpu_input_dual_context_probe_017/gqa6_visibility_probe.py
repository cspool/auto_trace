...
if sys.argv[1]=='parent':
 ...
 d=int(sys.argv[2]);original_input=Path(os.environ['HIPPROF_INPUT_FILE']);original_text=original_input.read_text();lines=original_text.splitlines();assert len([x for x in lines if x.startswith('gpu:')])==1;updated='\n'.join('gpu: '+str(d) if x.startswith('gpu:') else x for x in lines)+'\n';worker_input=BASE/('worker_input.rank'+str(d)+'.txt');worker_input.write_text(updated);os.environ['HIPPROF_INPUT_FILE']=str(worker_input)
 ...
 torch.cuda.set_device(1-d);other_context_tensor=torch.randn((1,),device="cuda");torch.cuda.synchronize();torch.cuda.set_device(d)
 graph=None
 if sys.argv[3]=='graph':
