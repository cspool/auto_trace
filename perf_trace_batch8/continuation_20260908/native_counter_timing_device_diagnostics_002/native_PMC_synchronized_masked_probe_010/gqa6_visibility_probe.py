...
if sys.argv[1]=='parent':
 ps=[subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct'],env={**os.environ,'HIP_VISIBLE_DEVICES':str(d),'CUDA_VISIBLE_DEVICES':str(d)}) for d in [0,1]]
 for d in [0,1]:wait('initial_stopped.rank'+str(d)+'.json')
 ...
 torch.cuda.set_device(0);other_context_tensor=torch.randn((1,),device="cuda");torch.cuda.synchronize();torch.cuda.set_device(0)
 graph=None
 if sys.argv[3]=='graph':
  graph=torch.cuda.CUDAGraph()
  with torch.cuda.graph(graph):launch()
 write('warm_ready.rank'+str(d)+'.json',{'rank':d,'pid':os.getpid()});wait('ALLOW_START')
