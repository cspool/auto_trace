...
base=R/'raw/runtime_tools/native_PMC_dual_context_external_probe_003';base.mkdir();(base/'hipprof_tmp').mkdir();original=(R/'raw/runtime_tools/native_fresh_exec_pmc_write_probe_001/gqa6_native_probe.py').read_text();worker=original[original.index('else:\n')+len('else:\n'):]
...
visibility=[read(base/n) for n in ['VISIBILITY_BEFORE_STOP.json','VISIBILITY_AFTER_STOP.json','VISIBILITY_AFTER_LIBC_FLUSH.json','VISIBILITY_AFTER_WORKER_EXIT.json']];save(base/'RESULT.json',{'status':'complete','model_initializations':0,'native_device_counts':dict(counts),'visibility':visibility,'argv':argv,'all_started_processes_terminated':True,'not_R08_model_capture_evidence':True,'each_worker_also_created_other_device_context_before_profiling':True,'native_kernel_profile_mode':1,'expected_4_per_device_observed':coverage_complete});print(json.dumps({'status':'complete_model_free_dual_context_observation','expected_4_per_device_observed':coverage_complete,'native_device_counts':dict(counts),'visibility':visibility}),flush=True)
