...
BRANCH='progress/batch8-continuation-20260908';DEADLINE=datetime.datetime.fromisoformat('2026-09-09T04:18:09+00:00').timestamp()
...
def snapshot():
 ...
 def copy(path,relative,kind):
  ...
  if path.stat().st_size>20_000_000:
   ...
   with path.open('rb') as large:
    size=os.fstat(large.fileno()).st_size;remaining=size
    while remaining:
     block=large.read(min(8<<20,remaining))
     if not block:break
     digest.update(block);remaining-=len(block)
   record={'source_path':str(path),'kind':'large_artifact_NFS_retained_for_Release','captured_bytes':size-remaining,'source_captured_prefix_sha256':digest.hexdigest(),'contents_in_Git':False,'full_contents_retained_on_NFS':True,'original_kind':kind};q=tmp/(str(relative)+'.LARGE_ARTIFACT_REFERENCE.json');q.parent.mkdir(parents=True,exist_ok=True);q.write_text(json.dumps(record,indent=2)+'\n');records.append(record);return
 ...
 for name in ['USER_RELEASE_OFFLOAD_POLICY_20260908.json','RELEASE_RESTORATION_CPU_GATE.json','CPU_SOURCE_AUTHORIZATION_CACHE_GATE.json']:
  copy(CONTROL/name,Path('state')/name,'closed_user_policy_or_CPU_gate')
 for p in sorted((R08/'tools').rglob('*')):
  if p.suffix in ['.py','.sh','.json','.js','.css','.md'] and p.stat().st_size<10_000_000:copy(p,Path('r08/frozen_tools')/p.relative_to(R08/'tools'),'frozen_stage_tool_or_gate')
 ...
 names=['session_state.json','MACHINE_TIME_EXTENSION_001.json','MACHINE_TIME_EXTENSION_002.json','MACHINE_TIME_EXTENSION_APPLIED_002.json','MACHINE_TIME_EXTENSION_ACTIVATED_002.json','WEIGHTS_COMPLETE.json','WEIGHT_STORAGE_RELOCATION_COMPLETE.json','MEMORY_STORAGE_RELOCATION_AUTHORIZATION.json','MEMORY_STORAGE_RELOCATION_COMPLETE.json','MEMORY_STORAGE_RELOCATION_AUTHORIZATION_002.json','MEMORY_STORAGE_RELOCATION_COMPLETE_002.json','R07_CPU_DB_REVIEW.md']
 for name in names:copy(CONTROL/name,Path('state')/name,'state_checkpoint')
 accepted=[]
 for schedule in sorted((R08/'validation').glob('serial_scheduler_*'))+sorted((R08/'raw/runtime_tools').glob('serial_scheduler_*')):
  for p in sorted(schedule.glob('*')):
   if p.suffix in ['.json','.log']:copy(p,Path('r08/scheduler')/schedule.name/p.name,'log_snapshot' if p.suffix=='.log' else 'closed_checkpoint')
   if p.name.endswith('.accepted_checkpoint.json'):
    item=json.loads(p.read_text())['item']
    if item['segment_id'] not in {x['segment_id'] for x in accepted}:accepted.append(item)
 for template_root in sorted(CONTROL.glob('stage_tool_templates_*')):
  for p in sorted(template_root.rglob('*')):
   if p.is_file() and p.suffix in ['.py','.js','.css','.json','.md']:copy(p,Path('code')/template_root.name/p.relative_to(template_root),'prepared_stage_template_code')
 for attempt in sorted((R08/'raw/captures').glob('*/*')):
  for p in sorted(set(attempt.glob('*.json'))|set((attempt/'control').rglob('*.json'))|set((attempt/'workload').glob('*.json'))):
   if p.stat().st_size<10_000_000:copy(p,Path('r08/capture_control')/p.relative_to(R08/'raw/captures'),'closed_or_live_control_snapshot')
