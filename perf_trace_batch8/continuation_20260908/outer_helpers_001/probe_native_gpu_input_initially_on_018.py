...
base=R/'raw/runtime_tools/native_PMC_gpu_input_initially_on_probe_018';base.mkdir();(base/'hipprof_tmp').mkdir();original=(R/'raw/runtime_tools/native_fresh_exec_pmc_write_probe_001/gqa6_native_probe.py').read_text();worker=original[original.index('else:\n')+len('else:\n'):]
worker=worker.replace(' assert lib.hipProfilerStop()==0\n print(',''' write('before_stop.rank'+str(d)+'.json',{'rank':d,'pid':os.getpid()});wait('ALLOW_STOP')
 ...
 print(''')
worker=worker.replace(' graph=None', ' torch.cuda.set_device(1-d);other_context_tensor=torch.randn((1,),device="cuda");torch.cuda.synchronize();torch.cuda.set_device(d)\n graph=None')
...
'''
worker=worker.replace(' sys.path.insert(0,str(TARGET))', " d=int(sys.argv[2]);original_input=Path(os.environ['HIPPROF_INPUT_FILE']);original_text=original_input.read_text();lines=original_text.splitlines();assert len([x for x in lines if x.startswith('gpu:')])==1;updated='\\n'.join('gpu: '+str(d) if x.startswith('gpu:') else x for x in lines)+'\\n';worker_input=BASE/('worker_input.rank'+str(d)+'.txt');worker_input.write_text(updated);os.environ['HIPPROF_INPUT_FILE']=str(worker_input)\n write('input_binding.rank'+str(d)+'.json',{'rank':d,'pid':os.getpid(),'original_input_path':str(original_input),'original_input_sha256':hashlib.sha256(original_text.encode()).hexdigest(),'worker_input_path':str(worker_input),'worker_input_sha256':hashlib.sha256(updated.encode()).hexdigest(),'only_gpu_selector_changed':True})\n sys.path.insert(0,str(TARGET))")
worker=worker.replace('lib.hipProfilerStop();','');script=base/'gqa6_visibility_probe.py';script.write_text(header+worker);compile(script.read_text(),str(script),'exec');save(base/'CPU_GATE.json',{'status':'complete','script':source_record(script),'prior_failed_capture':source_record(A/'RAW_CAPTURE_FAILURE.json'),'prior_groups_verified_closed':groups,'model_initializations':0,'four_measured_gqa6_dispatches_per_device_only':True,'native_counter_mode':'pmc_write','not_R08_model_measurement_evidence':True})
env=clean_env();env.pop('PYTHONPATH',None);env.update(TRITON_CACHE_DIR=str(R/'raw/runtime_cache/triton'),PYTHONPYCACHEPREFIX=str(R/'raw/runtime_cache/pycache'),PYTHONDONTWRITEBYTECODE='1',ROCPROFILER_FLUSH_INTERVAL='1000',HIP_VISIBLE_DEVICES='0,1',CUDA_VISIBLE_DEVICES='0,1');argv=collector_argv('pmc_write','_gqa6',base,[sys.executable,'-B',str(script),'parent','direct'],trace=True,disabled=False);forced=False
