...
base=R/'raw/runtime_tools/native_PMC_masked_worker_probe_005';base.mkdir();(base/'hipprof_tmp').mkdir();original=(R/'raw/runtime_tools/native_fresh_exec_pmc_write_probe_001/gqa6_native_probe.py').read_text();worker=original[original.index('else:\n')+len('else:\n'):]
...
text=script.read_text();text=text.replace("ps=[subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct']) for d in [0,1]]", "ps=[subprocess.Popen([sys.executable,'-B',__file__,'worker',str(d),'direct'],env={**os.environ,'HIP_VISIBLE_DEVICES':str(d),'CUDA_VISIBLE_DEVICES':str(d)}) for d in [0,1]]");text=text.replace('torch.cuda.set_device(d)','torch.cuda.set_device(0)').replace('torch.cuda.set_device(1-d)','torch.cuda.set_device(0)');script.write_text(text);compile(script.read_text(),str(script),'exec');save(base/'CPU_GATE.json',{'status':'complete','script':source_record(script),'prior_failed_capture':source_record(A/'RAW_CAPTURE_FAILURE.json'),'prior_groups_verified_closed':groups,'model_initializations':0,'four_measured_gqa6_dispatches_per_device_only':True,'native_counter_mode':'pmc_write','not_R08_model_measurement_evidence':True})
...
visibility=[read(base/n) for n in ['VISIBILITY_BEFORE_STOP.json','VISIBILITY_AFTER_STOP.json','VISIBILITY_AFTER_LIBC_FLUSH.json','VISIBILITY_AFTER_WORKER_EXIT.json']];save(base/'RESULT.json',{'status':'complete','model_initializations':0,'native_device_counts':dict(counts),'visibility':visibility,'argv':argv,'all_started_processes_terminated':True,'not_R08_model_capture_evidence':True,'each_worker_also_created_other_device_context_before_profiling':False,'each_worker_sees_only_own_physical_device':True,'original_other_context_touch_uses_visible_logical_device_zero':True,'expected_4_per_device_observed':coverage_complete});print(json.dumps({'status':'complete_model_free_dual_context_observation','expected_4_per_device_observed':coverage_complete,'native_device_counts':dict(counts),'visibility':visibility}),flush=True)
