...
def main():
 for gate in [ROOT/'raw/runtime_tools/runtime_capture_gate_014.json', ROOT/'validation/background_graph_analysis_CPU_gate_001.json',ROOT/'validation/independent_native_sessions_analysis_CPU_gate_001.json']:
  g=read(gate);assert g['status']=='complete'
  for x in g['frozen_tools']:assert sha(x['path'])==x['sha256'],'frozen tool bytes'
 SCHEDULE.mkdir();save(SCHEDULE/'START.json',{'pid':os.getpid(),'source':rec(Path(__file__)),'deadline_utc':'2026-09-09T04:18:09Z','deadline_authorization':rec(Path('/public/home/accl15ptg7/run_R08_R10/MACHINE_TIME_EXTENSION_002.json')),'previous_scheduler':rec(ROOT/'validation/serial_scheduler_012/START.json'),'resume_reason':'reuse eight audited captures; retry capture09 after rank1 native pre-measured health failure using native-probed runtime021 independent worker collectors with retained original sources and unchanged full workload','native_prehealth_retry_authorization':rec(ROOT/'raw/runtime_tools/capture09_independent_native_sessions_retry_001.json'),'NFS_storage_authorization':rec(ROOT/'authorization/NFS_output_storage_authorization_001.json'),'user_storage_policy':rec(Path('/public/home/accl15ptg7/run_R08_R10/USER_STORAGE_POLICY_20260908.json')),'scope':'R08 captures and CPU attribution; no R09/R10 business execution'})
 accepted=[]
 for ordinal,unit in enumerate(read(ROOT/'plans/r08_capture_plan.json')['physical_captures']):
  seg=unit['segment_id']
  if ordinal<8:
   previous=read(ROOT/'validation/serial_scheduler_012'/(seg+'.accepted_checkpoint.json'));assert previous['status']=='accepted';item=previous['item']
  ...
  attempt='attempt_006' if ordinal==8 else 'attempt_001';raw=ROOT/'raw/captures'/seg/attempt
  ...
  output=ROOT/'normalized/captures'/seg/attempt/'revision_002';phase([sys.executable,'-B',str(ROOT/'tools/analysis_008/normalize_capture.py'),'--segment',seg,'--attempt',attempt,'--revision','revision_002'],SCHEDULE/(seg+'.normalize.log'));phase([sys.executable,'-B',str(ROOT/'tools/analysis_008/audit_capture.py'),'--normalization',str(output)],SCHEDULE/(seg+'.audit.log'))
