...
R=Path('/public/home/accl15ptg7/auto_trace/perf_trace_batch8/runtime/workflow01-10-fresh-e2e/batch8-dp2-fresh-003/artifacts/R08/continuation_001');sys.argv=['normalize_capture.py','--segment','04_triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_0_pmc','--attempt','attempt_003','--revision','diagnostic_003'];started=time.monotonic()
try:runpy.run_path(str(R/'tools/analysis_006/normalize_capture.py'),run_name='__main__')
except ValueError as error:
 tb=error.__traceback__;s=None
 while tb:
  if tb.tb_frame.f_code.co_name=='main' and tb.tb_frame.f_code.co_filename.endswith('normalize_capture.py'):s=tb.tb_frame.f_locals;g=tb.tb_frame.f_globals
  tb=tb.tb_next
 ...
 for pid in s['pid_ranks']:
  ...
  for op in conn.execute('SELECT rowid AS native_rowid,* FROM '+g['q']('HIPOPS_'+ck)+' WHERE Name IN ('+','.join('?' for _ in ids)+')',ids):
   hs=list(conn.execute('SELECT rowid AS native_rowid,* FROM '+g['q']('HIP_'+ck)+' WHERE _Index=?',(op['_Index'],)))
   ...
   if api not in g['LAUNCH_APIS']:bad.append({'pid':pid,'op':dict(op),'HIP':h,'API':api,'native_ops_with_same_Index':conn.execute('SELECT COUNT(*) FROM '+g['q']('HIPOPS_'+ck)+' WHERE _Index=?',(op['_Index'],)).fetchone()[0],'required_selected_dispatch':(pid,op['_Index']) in eligible})
 ...
 print(json.dumps({k:v for k,v in result.items() if k!='non_direct_dispatches'},indent=2),flush=True);print('NON_DIRECT_RECORDS',len(bad),json.dumps(bad[:10],indent=2),flush=True)
