...
for p in (raw/'workload/overlay_events').glob('rank*/*.jsonl'):
 with p.open() as f:
  for line in f:
   x=json.loads(line)
   if x.get('marker_kind')=='process' and x['canonical_target_id'] in bound:markers[x['range_name']]=bound[x['canonical_target_id']]
...
unions={}
for scope,items in intervals.items():
 u=[]
 for row in items:
  if u and row[0]<=u[-1][1]:u[-1][1]=max(u[-1][1],row[1])
  else:u.append([row[0],row[1]])
 unions[scope]=u
begins={k:[x[0] for x in v] for k,v in unions.items()};seen=set();hits=[];cross=collections.Counter();groups=collections.Counter()
for x in d['non_direct_dispatches']:
 h=x['HIP'];scope=x['pid'],h['tid'];key=x['pid'],h['_Index'];groups[x['pid'],x['API']]+=1
 if key in seen:continue
 seen.add(key)
 if scope not in begins:continue
 pos=bisect.bisect_right(begins[scope],h['_Index'])-1
 if pos<0 or unions[scope][pos][1]<h['_Index']:continue
