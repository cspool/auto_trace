# Batch8 formal-runtime operational reference

Read relevant stage entries before fresh execution or runtime recovery. These
records preserve profiler/ABI, ownership, interruption and sampling lessons
from the original adaptation. Historical counts/attempt IDs are evidence for
the described runs, not replacement defaults for a new trace profile. The
active profile, frozen target manifest and scheduler state own current scope.
A presentation-only revision does not execute these runtime procedures.
