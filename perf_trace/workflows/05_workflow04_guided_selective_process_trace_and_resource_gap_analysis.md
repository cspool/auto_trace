...
单 batch fresh 配置绑定物理 DCU 1，`HIP_VISIBLE_DEVICES=1`、
`CUDA_VISIBLE_DEVICES=1`。输入、模型、采样、warmup 和完整请求范围由同一
run 的语义合同冻结，不从展示页反向修改采集参数。

配置见 [`configs/workflow01_10_fresh_e2e_dcu1.json`](../configs/workflow01_10_fresh_e2e_dcu1.json)。代码/skill/Workflow 更新后，新的执行使用当前
...
本节是 20260910 谱系的实测结论，属于上表 w05 覆盖的 R06–R08 规划依据，
**不是独立入口**。它回答"要出这两个页面，必须采什么、怎么采、哪里会踩坑"，
用于在 R06 规划阶段就把范围定对，而不是采完才发现视图缺必需证据。
...
按[执行指南](../scripts/process_pile_assets/README.md)恢复归档并生成当前双视图。此路径读取已接受源文件，
