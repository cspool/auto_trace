...
p(c)——这就是 <b>PLAS</b>（式 1：p(c_j) = Σ 同程序先前调用运行时，值大 = 优先级低，"已经花了多少"
替代"还要花多少"）；② 顺红线按 p(c) <b>直接落进 Q1–QK 对应队</b>（与传统 MLFQ 全从 Q1 开始不同——
这一步正是治"长程序新调用插队"的药）；③ 量子用尽向低优先级降级；④ β 反饥饿拉回 Q1。多线程程序
用 <b>ATLAS</b>（式 2：p(c_j) = max 父{p(c_k)+t_k}，单个最长观测关键路径标量近似 DAG 关键路径，
...
等待/服务比 (W_p+W_c)/(T_p+T_c)，≥ β 才把该调用提回 Q1，且只重置本调用的 W_c/T_c（β 调节均值
