...
## Analytical view plan and resource sidecars

The twelve base tables remain complete. Produce or explicitly bind a derived
view plan using the shared
[Process/resource contract](../../qwen-dcu-workflow05-trace-visualization-reporting/references/process-resource-contract.md).
Separate original high-latency classification from the type-duration display
threshold. Record type totals, pile memberships/global ranks, source scope and
metric-specific availability/coverage. Retain native replay times for same-pass
rates, without admitting them into R07 schedule arithmetic. Preserve valid zero,
null and the non-measured epsilon display annotation as different concepts.
These sidecars do not replace or filter the base tables.
...
All twelve tables must exist and their counts must equal their CSV row
counts; an empty table is valid only when justified by its source universe. Numeric gate values above illustrate the committed fresh-run
...
clock/alignment failure, silent row loss, dependency/model omission, hand-edited output, or any GPU
