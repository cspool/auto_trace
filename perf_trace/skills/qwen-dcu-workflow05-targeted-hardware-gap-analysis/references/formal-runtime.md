...
## Same-pass resource evidence

For every physical dispatch, preserve raw counters, native begin/end, units,
clock/pass identity, exact R07 join keys, shapes and missing states. Use the
resource formulas and attribution gates in the shared
[Process/resource contract](../../qwen-dcu-workflow05-trace-visualization-reporting/references/process-resource-contract.md#3-r09-analyses-and-derived-resource-values).
Replay duration is valid only for its own resource rate, never observed R07
latency. A bounded family plan is not all-Process hardware coverage.
...
traffic_boundary.hbm_or_dram_traffic_claimed=<true only for verified native counters, otherwise false>
